...
@Component({
  selector: 'stc-progress',
  templateUrl: './stc-progress.component.html',
  styleUrls: ['./stc-progress.component.scss']
})
export class PagestcComponent implements OnInit {
  pageTitle: string = 'STC';
  location: any[] = [];
  assigned: any = [];
  ...
  manipulateData() {
    this.assigned = _.filter(this.location, { assigned_to: 'STC' });

    for (var i = 0; i < this.location.length; i++) {
      for (var j = 0; j < this.location[i].status.length; j++) {
        if (this.location[i].status[j].operator === "STC")
  ...
  dataIntoCharts_area(area, stat, ndx) {
    ...
    var civil = _.filter(this.location, function (loc) {
      return _.some(loc.area === area && loc.status, { 'civil_status': stat, 'operator': 'STC' });
    });
    ...
    if (ndx === 0 || ndx === 1) {
      ...
      var coverage = _.filter(this.location, function (loc) {
        return _.some(loc.area === area && loc.status, { 'coverage_status': cov, 'operator': 'STC' });
      });
      this.barChartData[ndx]['data'].push(coverage.length);
    }
    var onair = _.filter(this.location, function (loc) {
      return _.some(loc.area === area && loc.status, { 'onAir_status': stat, 'operator': 'STC' });
    });
  ...
  dataIntoCharts_total(stat) {
    var cov = "";
    var civil = _.filter(this.location, function (loc) {
      return _.some(loc.status, { 'civil_status': stat, 'operator': 'STC' });
    });
    ...
    if (stat === "Completed" || stat === "Not Yet Started") {
      if (stat === "Completed")
        ...
        cov = "No";
      var coverage = _.filter(this.location, function (loc) {
        return _.some(loc.status, { 'coverage_status': cov, 'operator': 'STC' });
      });
      this.polarAreaChartData.push(coverage.length);
    }
    var onair = _.filter(this.location, function (loc) {
      return _.some(loc.status, { 'onAir_status': stat, 'operator': 'STC' });
    });
