...
@Component({
  selector: 'mobily-progress',
  templateUrl: './mobily-progress.component.html',
  styleUrls: ['./mobily-progress.component.scss']
})
export class PageMobilyComponent implements OnInit {
  pageTitle: string = 'Mobily';
  location: any[] = [];
  assigned: any = [];
  ...
  manipulateData() {
    this.assigned = _.filter(this.location, { assigned_to: 'Mobily' });

    for (var i = 0; i < this.location.length; i++) {
      for (var j = 0; j < this.location[i].status.length; j++) {
        if (this.location[i].status[j].operator === "Mobily")
          this.Mobily.push({ "sub": this.location[i].sub_area, "name": this.location[i].location_title, "civil": this.location[i].status[j].civil_status, "coverage": this.location[i].status[j].coverage_status, "air": this.location[i].status[j].onAir_status });
      }
    }
  ...
  civilStatus(subarea, stat) {
        return _.filter(this.Mobily, { "sub": subarea, "civil": stat });
      }
      coverStatus(subarea, stat) {
        return _.filter(this.Mobily, { "sub": subarea, "coverage": stat });
      }

  //based on sub area
  dataIntoCharts_sub(sub, stat, ndx) {
    var total = _.filter(this.Mobily, { sub: sub, air: stat });
  ...
  dataIntoCharts_area(area, stat, ndx) {
    ...
    var civil = _.filter(this.location, function (loc) {
      return _.some(loc.area === area && loc.status, { 'civil_status': stat, 'operator': 'Mobily' });
    });
    ...
    if (ndx === 0 || ndx === 1) {
      ...
      var coverage = _.filter(this.location, function (loc) {
        return _.some(loc.area === area && loc.status, { 'coverage_status': cov, 'operator': 'Mobily' });
      });
      this.barChartData[ndx]['data'].push(coverage.length);
    }
    var onair = _.filter(this.location, function (loc) {
      return _.some(loc.area === area && loc.status, { 'onAir_status': stat, 'operator': 'Mobily' });
    });
  ...
  dataIntoCharts_total(stat) {
    var cov = "";
    var civil = _.filter(this.location, function (loc) {
      return _.some(loc.status, { 'civil_status': stat, 'operator': 'Mobily' });
    });
    ...
    if (stat === "Completed" || stat === "Not Yet Started") {
      if (stat === "Completed")
        ...
        cov = "No";
      var coverage = _.filter(this.location, function (loc) {
        return _.some(loc.status, { 'coverage_status': cov, 'operator': 'Mobily' });
      });
      this.polarAreaChartData.push(coverage.length);
    }
    var onair = _.filter(this.location, function (loc) {
      return _.some(loc.status, { 'onAir_status': stat, 'operator': 'Mobily' });
    });
