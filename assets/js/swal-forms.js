    ...
    getFormValues: function (isConfirm) {
      ...
      function toValuableAttrs (tag) {
        ...
        if (isConfirm && tag.dataset.swalFormsRequired && !tag.value) {
          var warnMsg = 'Missing required attribute: ' + (tag.name || tag.placeholder)
      ...
      toHtml: function () {
        ...
        if (input.type !== 'select') {
          inputTag = t("<input id='{id}' class='{clazz} swal-form-field' type='{type}' name='{name}'" +
            " value='{value}' title='{placeholder}' placeholder='{placeholder}'{disabled}" +
            ' data-swal-forms-required={required}>', input)
        } else {
          inputTag = t("<br><select id='{id}' class='{clazz} swal-form-field' name='{name}'" +
            " value='{value}' title='{placeholder}' {disabled} style='float:left; width:426px; height:44px'>" +
            ' data-swal-forms-required={}', input) +
              input.options.reduce(toHtmlOptions, '') +
            '</select>'
        }
        var labelTag = t("<b><b><b><label for='{id}'>{label}</label><b>", input)
